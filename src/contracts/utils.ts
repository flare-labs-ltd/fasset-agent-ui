 ...
 * @param contractMethod contract method
 * if DEBUG_MODE=true we console info about contract call
 */
export async function logContractCall(message: string, contractName: string, contractMethod: string) {
    if (process.env.DEBUG_MODE) {
        let debug = false;
        try {
            debug = JSON.parse(process.env.DEBUG_MODE);
        } catch {
            debug = false;
        }
        if (debug == true) {
            console.info(`${contractName}.${contractMethod} ("${message}")`);
        }
    }
}
 ...
 * if DEBUG_MODE=true we console info about contract call
 */
export async function logFlarejsCall(message: string, functionCall: string) {
    if (process.env.DEBUG_MODE) {
        let debug = false;
        try {
            debug = JSON.parse(process.env.DEBUG_MODE);
        ...
        if (debug == true) {
            console.info(`flarejs: ${functionCall} ("${message}")`);
        }
    }
}
